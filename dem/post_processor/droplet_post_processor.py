...
class DropletPostProcessor(PostProcessor):
    ...
    def initialize_live_plot(self):
        ...
        self.circles = []
        for pos, r in zip(current_positions, radii, strict=False):
    ...
    def update_circle_positions(self, circle_set, position, radius):
        ...
        y = radius * np.sin(u) + position[1]
        z = np.full_like(x, position[2])
        circle_set[0].set_segments([list(zip(x, y, z, strict=False))])

        # Обновление окружности в плоскости XZ
        x = radius * np.cos(u) + position[0]
        y = np.full_like(x, position[1])
        z = radius * np.sin(u) + position[2]
        circle_set[1].set_segments([list(zip(x, y, z, strict=False))])

        # Обновление окружности в плоскости YZ
        x = np.full_like(u, position[0])
        y = radius * np.cos(u) + position[1]
        z = radius * np.sin(u) + position[2]
        circle_set[2].set_segments([list(zip(x, y, z, strict=False))])
    ...
    def plot_radius_percentiles_evolution(self, percentiles=None, show_min=True, show_max=True):
        """
        ...
        for p in percentiles:
            plt.plot(
                times, percentiles_radii[p], label=f"{p}th Percentile", linestyle="-", marker="o"
            )
    ...
    def plot_radius_evolution(
        ...
        for _ind, t in enumerate(times):
            ...
            for p in percentiles:
                ...
                percentile_volume = sorted_volumes[
                    np.searchsorted(cumulative_volumes, target_volume)
                ]
        ...
        for p in reversed(percentiles):
            plt.plot(
                times,
                np.array(percentiles_radii[p]) / unit_scale,
                label=f"{p}th Volume Percentile",
                linestyle="-",
                marker="o",
            )
        ...
        if plt_data_save:
            ...
            df = pd.DataFrame(
                {"Time, s": np.array(times).flatten(), r"Min Radius, $\mu$m": min_radii}
            )
            for p in percentiles:
    ...
    def plot_radius_evolution_old(
        self, percentiles=None, show_min=True, show_max=True, plt_show=True, plt_data_save=False
    ):
        """
        Строит график минимального и максимального радиусов и объёмных процентилей от времени по всей цепочке решений.
    ...
    def plot_number_of_droplets(
        self, time_window=None, time_step=None, plt_show=True, plt_data_save=False
    ):
        """
    ...
    def compare(
        self, other_solution, time_window=None, time_step=None, plt_show=True, plt_data_save=False
    ):
        """
        ...
        time_window = [
            max(
                time_window[0],
                self.solution.get_chain_start_time(),
                other_solution.get_chain_start_time(),
            ),
            min(
                time_window[1],
                self.solution.get_chain_end_time(),
                other_solution.get_chain_end_time(),
            ),
        ]
        ...
        for ind_t, t in enumerate(times):
            ...
            positions1_padded = np.pad(
                positions1, ((0, max_particles1 - len(positions1)), (0, 0)), "constant"
            )
            positions2_padded = np.pad(
                positions2, ((0, max_particles2 - len(positions2)), (0, 0)), "constant"
            )
    ...
    def plot_radius_ratio_statistics(
        self, window_size=10, overlap=0.5, ratio_intervals=None, plt_show=True, plt_data_save=False
    ):
        """
        ...
        for idx, window_start in enumerate(times):
            ...
            total_collisions = np.sum(
                [ratio_counts[i][idx] for i in range(len(ratio_intervals) + 1)]
            )
        ...
        for i, interval in enumerate(ratio_intervals):
            if i == 0:
                ...
                plt.plot(
                    times + window_size / 2,
                    ratio_counts[i],
                    label=f"{ratio_intervals[i-1]} < Ratio <= {interval}",
                )
        plt.plot(
            times + window_size / 2,
            ratio_counts[len(ratio_intervals)],
            label=f"{ratio_intervals[-1]} < Ratio",
        )
    ...
    def plot_angle_statistics(
        self, window_size=10, overlap=0.5, angle_intervals=None, plt_show=True, plt_data_save=False
    ):
        """
        ...
        for idx, window_start in enumerate(times):
                ...
                if window_start <= time <= window_end and current_solution.is_collision:
                    for pair in current_solution.collided_droplets:
                        ...
                        # Если нет предпоследнего шага, то вычисляем только по последнему
                            ...
                            pos1 = np.array(
                                [
                                    current_solution.trajectories[-1, pair[0], 0],
                            ...
                            pos2 = np.array(
                                [
                                    current_solution.trajectories[-1, pair[1], 0],
                        ...
                        cos_theta = np.dot(vector, z_vector) / (
                            np.linalg.norm(vector) * np.linalg.norm(z_vector)
                        )
            ...
            # Нормируем по числу столкновений в окне
            total_collisions = np.sum(
                [angle_counts[i][idx] for i in range(len(angle_intervals) + 1)]
            )
            if total_collisions > 0:
        ...
        for i, interval in enumerate(angle_intervals):
            if i == 0:
                ...
                plt.plot(
                    times + window_size / 2,
                    angle_counts[i],
                    label=f"{angle_intervals[i-1]}° < Angle <= {interval}°",
                )
        plt.plot(
            times + window_size / 2,
            angle_counts[len(angle_intervals)],
            label=f"{angle_intervals[-1]}° < Angle",
        )
    ...
    def plot_angle_statistics_in_given_intervals(
        self, windows, angle_intervals=None, plt_show=True, plt_data_save=False
    ):
        """
        ...
        for idx, (window_start, window_end) in enumerate(
            zip(windows[:-1], windows[1:], strict=False)
        ):
            current_solution = first_solution
                ...
                if window_start <= time <= window_end and current_solution.is_collision:
                    for pair in current_solution.collided_droplets:
                        ...
                        if current_solution.trajectories.shape[0] >= 2:
                            ...
                            # Линейная интерполяция для нахождения момента контакта
                            t_contact = (current_distance - collision_distance) / (
                                current_distance - np.linalg.norm(pos2_prev - pos1_prev)
                            )
        ...
        for i, interval in enumerate(angle_intervals):
            if i == 0:
                plt.plot(
                    (np.array(windows[:-1]) + np.array(windows[1:])) / 2,
                    angle_counts[i],
                    label=f"Angle <= {interval}°",
                )
            else:
                plt.plot(
                    (np.array(windows[:-1]) + np.array(windows[1:])) / 2,
        ...
        if plt_data_save:
            ...
            folder = "results/exportet_data"
            fname_to_save = (
                f"{folder}/{self.solution.get_name()}___angle_statistics_in_given_intervals.xlsx"
            )
            if not os.path.exists(folder):
                os.makedirs(folder)
            ...
            df = pd.DataFrame(
                {"Start time": np.array(windows[:-1]), "End time": np.array(windows[1:])}
            )
            for i, interval in enumerate(angle_intervals):
                if i == 0:
                    df[f"Angle <= {interval}°"] = angle_counts[i]
