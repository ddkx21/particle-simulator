"""
Запуск системы 2 (крупные капли).

  python -m dual_ensemble.run_system2                  # свежий запуск
  python -m dual_ensemble.run_system2 --continue       # продолжить с checkpoint
  python -m dual_ensemble.run_system2 --t-stop 150     # переопределить t_stop
"""
...
SYS_PREFIX = "sys2"
CHECKPOINT_FILE = os.path.join(RESULTS_DIR, "sys2_checkpoint.npz")
SNAPSHOTS_FILE = os.path.join(RESULTS_DIR, "sys2_snapshots.npz")


def main() -> None:
    import taichi as ti

    parser = argparse.ArgumentParser(description="Запуск системы 2")
    ...
    _, N2, box_size = compute_system_params(cfg)
    ...
    print(f"\nСистема 2: N={N2}, radii=[{radii_range[0]*1e6:.1f}, {radii_range[1]*1e6:.1f}] мкм")
    ...
    print(f"\nДля продолжения: python -m dual_ensemble.run_system2 --continue")
