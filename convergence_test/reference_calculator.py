...
Использует DirectDropletForceCalculator с boundary_mode="open"
на полной суперячейке, затем извлекает результаты для центральных N частиц.
"""
import numpy as np
import taichi as ti
...
def compute_reference(positions: np.ndarray, radii: np.ndarray,
    ...
    суперячейки (прямой расчёт без MIC, без поправки).
    ...
    # Создаём калькулятор без периодики и без поправки
    calc = DirectDropletForceCalculator(
        num_particles=N_total,
        eps_oil=eps_oil, eta_oil=eta_oil, E=E,
        L=L * K,  # размер суперячейки (не используется при open)
        boundary_mode="open",
